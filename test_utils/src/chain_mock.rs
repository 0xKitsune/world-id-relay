...
use ethers::prelude::{
    Http, LocalWallet, NonceManagerMiddleware, Provider, Signer, SignerMiddleware, Wallet,
};
use ethers::providers::Middleware;
use ethers::types::{Uint8, U256};
use ethers::utils::{Anvil, AnvilInstance};

use crate::abi::{MockBridgedWorldID, MockStateBridge, MockWorldID};

pub const TREE_DEPTH: u8 = 30;

pub type TestMiddleware =
...
pub async fn spawn_mock_chain() -> eyre::Result<MockChain<TestMiddleware>> {
    ...
    let initial_root = U256::from_str("0x111").expect("couldn't convert hex string to u256");
